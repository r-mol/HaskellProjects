...
run :: IO ()
run = activityOf (initUniverse sampleStarSystem) handleUniverse drawUniverse
...
instance Semigroup Field where
  Field f1 <> Field f2 = Field (\p -> f1 p `vectorSum` f2 p)

instance Monoid Field where
  mempty = Field (\_point -> (0, 0))
...
instance Semigroup Bounds where
  Unbounded <> x = x
  x <> Unbounded = x
  Rectangular (l1, b1) (r1, t1) <> Rectangular (l2, b2) (r2, t2)
    = Rectangular
        (min l1 l2, min b1 b2)
        (max r1 r2, max t1 t2)
...
bodyBounds body = Rectangular (l, b) (r, t)
  where
...
systemField bodies = mconcat (map bodyField bodies)
